...
import { groupThousands } from '../lib/format.js'
import logoUrl from '../assets/signet-logo.svg'

// Count-up number. `format` maps the eased value to display text.
export function AnimatedNumber({ value, format, duration = 1.6, delay = 0, className }) {
  const reduce = useReducedMotion()
  const mv = useMotionValue(reduce ? value : value === 0 ? 0 : 10000)
  const fmt = format || ((v) => groupThousands(v, ' '))
  ...
  }, [value, reduce, duration, delay]) // eslint-disable-line
